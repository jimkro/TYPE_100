...
export function draw(canvas, ctx) {
    ...
    state.fireZones.forEach(z => {
        // [進化視覺] 如果是進化火焰 (isEvolved)，顯示為藍色/紫色火焰
        if (z.isEvolved) {
            ctx.fillStyle = `rgba(50, 100, 255, ${0.4 + Math.random()*0.3})`; // 藍火
        } else {
            ctx.fillStyle = `rgba(255, 100, 0, ${0.3 + Math.random()*0.2})`; // 一般火
        }
        ctx.beginPath(); ctx.arc(z.x, z.y, z.radius, 0, Math.PI*2); ctx.fill();
    });

    // Enemies (包含 Boss 顯示，同前)
    ...
    state.projectiles.forEach(p => {
        // [進化視覺]
        if (p.isEvolved) {
            if (p.weaponId === 'gun') ctx.fillStyle = '#0ff'; // 青色雷射
            else if (p.weaponId === 'bow') ctx.fillStyle = '#fa0'; // 金色箭矢
            else ctx.fillStyle = '#fff';
            
            ctx.beginPath(); ctx.arc(p.x, p.y, 8, 0, Math.PI*2); ctx.fill(); // 比較大顆
        } else {
            ctx.fillStyle = '#ff0'; 
...
// (drawMoveHints 和 drawMinimap 保持原樣)
