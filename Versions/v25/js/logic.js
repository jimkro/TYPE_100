    ...
    if (cur.id === 'bow' && isEvolved) {
        ...
        angles.forEach(angle => {
            ...
            state.projectiles.push({
                ...
                prevX: player.x, prevY: player.y, // [新增] 平滑渲染初始值
                vx: nvx, vy: nvy,
...
export function updatePhysics(canvasWidth, canvasHeight) {
    ...
    // --- [新增] 1. 平滑渲染快照 (Snapshot) ---
    // 在運算前保存當前位置，供 renderer.js 進行內插
    player.prevX = player.x;
    player.prevY = player.y;
    state.camera.prevX = state.camera.x;
    state.camera.prevY = state.camera.y;

    enemies.forEach(e => { e.prevX = e.x; e.prevY = e.y; });
    projectiles.forEach(p => { p.prevX = p.x; p.prevY = p.y; });
    enemyProjectiles.forEach(p => { p.prevX = p.x; p.prevY = p.y; });
    ...
    // --- [優化] Spawning 難度曲線 ---
    // 1. 限制最大數量，避免後期太卡
    const MAX_ENEMIES = 50 + Math.floor(player.level * 2);

    // 2. 動態生成機率：Lv1 很慢(0.5%) -> Lv30 正常(3.5%)
    let spawnChance = 0.005 + (player.level * 0.001);
    spawnChance = Math.min(spawnChance, 0.04); // 鎖定上限 4%

    if (enemies.length < MAX_ENEMIES && Math.random() < spawnChance) {
        let ex, ey, d;
        ...
        if(w) {
            // [優化] 速度曲線
            // 基礎速度從 0.8 降為 0.35 (新手友善)
            // 成長幅度從 0.12 降為 0.015 (緩慢變難)
            const baseSpeed = 0.35 + Math.random()*0.4 + (player.level * 0.015);
            
            const isShooter = (player.level >= 5 && Math.random() < 0.2); // Lv5 後才出現射手
            
            enemies.push({ 
                x:ex, y:ey, 
                prevX: ex, prevY: ey, // [新增]
                size:25, 
        ...
        if(e.isShooter && onScreen) {
            ...
            if(e.shootTimer<=0) {
                state.enemyProjectiles.push({
                    x:e.x, y:e.y, 
                    prevX:e.x, prevY:e.y, // [新增]
                    vx:(dx/dist)*2.5, vy:(dy/dist)*2.5, // 稍微降低子彈速度 
                    life:600
                });
...
function spawnBoss(playerLevel) {
    ...
    state.enemies.push({ 
        x: ex, y: ey, 
        prevX: ex, prevY: ey, // [新增]
        size: 60, color: '#ffd700', word: w, speed: 0.6 + (playerLevel * 0.05), burnTimer: 0, isShooter: true, shootTimer: 100, isBoss: true, lives: bossLives, maxLives: bossLives 
    });
