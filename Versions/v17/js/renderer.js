...
export function draw(canvas, ctx) {
    ...
    state.enemyProjectiles.forEach(ep => {
        ...
        if(Math.abs(ep.y - state.player.y) > canvas.height/2 + 50) return;
    ...
    // Input Box (位置上移，避免擋住上方箭頭)
    // 原本是 p.y-55, 改為 p.y-90
    ctx.fillStyle='rgba(0,0,0,0.7)'; ctx.fillRect(p.x-60, p.y-102, 120, 30);
    ctx.fillStyle='#fff'; ctx.font='20px Courier New'; ctx.textAlign='center';
    // 文字位置配合調整
    ctx.fillText(state.currentInput, p.x, p.y-100);
